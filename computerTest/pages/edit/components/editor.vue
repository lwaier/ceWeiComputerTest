		...
		<view class="page-body">
			<view class='wrapper'>
				...
				<editor id="editor" class="ql-container" placeholder="开始输入..." showImgSize showImgToolbar showImgResize
				 @statuschange="onStatusChange" :read-only="readOnly" @ready="onEditorReady" @input="getEditerValue">
				</editor>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
					...
					success: (res) => {
						...
						uni.uploadFile({
							...
							success: function(res) {
								...
								if(data.code == '000000'){
									let imgSrc = data.result.substring(7).split('\\').join('/');
									console.log(imgSrc,'图片地址')
									console.log(that.IP+imgSrc,'图片地址')
			...
			getEditerValue(e){
				console.log(e.detail, '获取到的富文本信息')
				let detailJSON =  JSON.stringify(e.detail);
				this.$emit('getDetail',detailJSON);
			}
		},
		onLoad() {
			console.log(100)
